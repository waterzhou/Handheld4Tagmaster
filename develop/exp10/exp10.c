...
int main(int argc, char **argv)
{
	...
	ioctl(fd1, FBIOGET_VSCREENINFO, &var_info);	
	...
	var_info.xoffset = 0;
	var_info.yoffset = 0;
	ioctl(fd1, FBIOPAN_DISPLAY, &var_info);
	
	screen_mm = (char *)mmap(0, buff_size, PROT_READ|PROT_WRITE, MAP_SHARED, fd1, 0);
	bzero(screen_mm, buff_size);

	fd2 = open("/dev/zlg7289", O_RDWR);
	if(fd2 == -1){
		printf("open /dev/zlg failed\n");
		exit(1);
	}
	...
	while(1){	
		...
		if(databuf[0] != 0xff){
			...
			text[0] = databuf[0]+0x30;
			text[1] = '\0';
			textout(screen_mm, xres, yres, text, 1, 1);
			xres += 8;
			if(xres >= 160){
				xres = 0;
				yres += 16;
