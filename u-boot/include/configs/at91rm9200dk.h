...
#define CONFIG_EXTRA_ENV_SETTINGS \
  ...
  "ethaddr=00:18:58:00:00:00\0" \
  "mtdparts=physmap-flash.0:0x20000@0x0(bootbin),0x20000@0x20000(boot)," \
    "0x20000@0x40000(env),0x2A0000@0x60000(kernel),-@0x300000(fs)\0" \
  "bootcmd_flash=protect off 10000000 10ffffff; bootm 10060000\0" \
  "bootcmd_net=tftp 21000000 $(kernel); protect off 10000000 10ffffff; bootm 21000000\0" \
  "boot_flash=setenv bootargs mem=32M console=ttyS0,115200 " \
    "root=/dev/mtdblock4 rw rootfstype=jffs2 " \
    "mtdparts=$(mtdparts) noinitrd " \
    "tm=$(ethaddr)/$(ipaddr); run bootcmd_flash\0" \
  "boot_net=setenv bootargs mem=32M console=ttyS0,115200 root=/dev/nfs " \
    "rw nfsroot=\$(serverip):$(nfsroot) ip=\$(ipaddr):\$(serverip) " \
    "mtdparts=$(mtdparts) tm=$(ethaddr)/$(ipaddr); "\
    "run bootcmd_net\0" \
  "bootcmd=run boot_flash\0" \
  "fastboot=on\0"

#define CONFIG_ENV_OVERWRITE 1 /* allow 'ethaddr' to be overwritten */
