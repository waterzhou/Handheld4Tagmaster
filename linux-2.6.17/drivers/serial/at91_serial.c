...
static void at91_stop_tx(struct uart_port *port)
{
	...
	if (at91_port->use_dma_tx) {
		...
		UART_PUT_IDR(port, AT91_US_ENDTX | AT91_US_TXBUFE);
	}
	else
...
static void at91_start_tx(struct uart_port *port)
{
	...
	if (at91_port->use_dma_tx) {
		...
		UART_PUT_IER(port, AT91_US_ENDTX | AT91_US_TXBUFE);
...
static void at91_stop_rx(struct uart_port *port)
{
	...
	if (at91_port->use_dma_rx) {
		UART_PUT_PTCR(port, AT91_PDC_RXTDIS);		/* disable PDC receive */
		UART_PUT_IDR(port, AT91_US_ENDRX | AT91_US_TIMEOUT);
	}
	else
...
static void at91_pdc_endrx(struct uart_port *port)
{
	...
	struct at91_dma_buffer *pdc = PDC_RX_BUF(at91_port);
	unsigned int count;
	...
	PDC_RX_SWITCH(at91_port);		/* next PDC buffer */
}
...
static void at91_pdc_timeout(struct uart_port *port)
{
	...
	UART_PUT_CR(port, AT91_US_STTTO);
}
...
static void at91_pdc_endtx(struct uart_port *port)
{
	...
	struct at91_dma_buffer *pdc = &at91_port->pdc_tx;
...
static irqreturn_t at91_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	while (pending) {
		/* PDC receive */
		if (pending & AT91_US_ENDRX)
			at91_pdc_endrx(port);
		if (pending & AT91_US_TIMEOUT)
			at91_pdc_timeout(port);
		if (at91_port->use_dma_rx && pending & (AT91_US_RXBRK | AT91_US_OVRE | AT91_US_FRAME | AT91_US_PARE)) 
		...
		/* PDC transmit */
		if (pending & AT91_US_ENDTX)
			at91_pdc_endtx(port);
		if (pending & AT91_US_TXBUFE)
...
static int at91_startup(struct uart_port *port)
{
	...
	if (at91_port->use_dma_rx) {
		...
		for (i = 0; i < 2; i++) {
			struct at91_dma_buffer *pdc = &at91_port->pdc_rx[i];

			pdc->buf = kmalloc(PDC_BUFFER_SIZE, GFP_KERNEL);
			if (pdc->buf == NULL) {
				if (i != 0) {
					dma_unmap_single(port->dev, at91_port->pdc_rx[0].dma_addr, PDC_BUFFER_SIZE, DMA_FROM_DEVICE);
					kfree(at91_port->pdc_rx[0].buf);
				}
		...
		at91_port->pdc_rx_idx = 0;

		UART_PUT_RPR(port, at91_port->pdc_rx[0].dma_addr);
		UART_PUT_RCR(port, PDC_BUFFER_SIZE);

		UART_PUT_RNPR(port, at91_port->pdc_rx[1].dma_addr);
	...
	if (at91_port->use_dma_rx) {
		...
		UART_PUT_IER(port, AT91_US_ENDRX | AT91_US_TIMEOUT);
...
#ifdef SUPPORT_PDC
	at91_port->use_dma_rx = data->use_dma_rx;
	at91_port->use_dma_tx = data->use_dma_tx;
#endif
}
